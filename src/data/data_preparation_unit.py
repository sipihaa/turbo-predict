...
class Input_Gen(object):
    ...
    def __init__(self, df_train, df_test, cols_normalize, sequence_length, sequence_cols, sample_dir_path,
        ...
        train_data_to_scale = df_train[cols_normalize].values
        scaled_train_data = min_max_scaler.fit_transform(train_data_to_scale)
        norm_df = pd.DataFrame(scaled_train_data,
        ...
        test_data_to_scale = df_test[cols_normalize].values
        scaled_test_data = min_max_scaler.transform(test_data_to_scale)
        norm_test_df = pd.DataFrame(scaled_test_data,
                                    columns=cols_normalize,
                                    index=df_test.index)
        ...
        # print("the number of input signals: ", len(cols_normalize))
        # min_max_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1))
        # train_data_to_scale = df_train[cols_normalize].copy()
        # train_data_to_scale.columns = train_data_to_scale.columns.astype(str)
        # norm_df = pd.DataFrame(min_max_scaler.fit_transform(train_data_to_scale),
        #                        columns=cols_normalize,
        #                        index=df_train.index)
        # join_df = df_train[df_train.columns.difference(cols_normalize)].join(norm_df)
        # df_train = join_df.reindex(columns=df_train.columns)

        # test_data_to_scale = df_test[cols_normalize].copy()
        # test_data_to_scale.columns = test_data_to_scale.columns.astype(str)
        # norm_test_df = pd.DataFrame(min_max_scaler.transform(test_data_to_scale), columns=cols_normalize,
        #                             index=df_test.index)
        # test_join_df = df_test[df_test.columns.difference(cols_normalize)].join(norm_test_df)
        # df_test = test_join_df.reindex(columns=df_test.columns)
        # df_test = df_test.reset_index(drop=True)
